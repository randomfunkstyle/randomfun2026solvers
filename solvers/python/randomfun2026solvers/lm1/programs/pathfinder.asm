...
; No coprocessor: the CPU drives the LM-75 itself through DSPA/DSPD/DSPS.
; That costs three opcodes where a write-only unit would cost one `SND`, so the
; trie is depth 5 rather than 4 — deliberately, to get a *running* machine on
; hardware that is already verified (plotter, palette, snake all paint this way).
...
; ══ setup round: 256 cells, then the robot ═════════════════════════════
        LDI 0
        ST  ZERO
; home the write cursor once; DATA then advances by itself (§4.4)
        LDI 0
        DSPA
...
rs5:
        ...
        DIVI 2
        ST  E
; paint the robot and commit — that is the setup round's single frame
        LD  RP
        DSPA
        LDI 10
        DSPD
