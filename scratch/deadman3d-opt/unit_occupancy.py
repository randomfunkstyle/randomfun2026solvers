"""What each DOOM-unit arm actually occupies, against the 20 columns it is given.
...
region map's nominal LEAF_PITCH box.
"""
from randomfun2026solvers.lm1 import d3_unit as U

unit = U.unit_interior()
cells = unit.cells
cols = U.arm_columns()
codes = U.arm_codes()

print(f"interior {U.UNIT_IW} x {U.UNIT_IH}   trie leaves at "
      f"{[U.LEAF0 + U.LEAF_PITCH * i for i in range(8)]}")
...
for i, (arm, x0) in enumerate(order):
    x1 = order[i + 1][1] if i + 1 < len(order) else U.UNIT_IW + 1
...
print(f"\nreal column span, summed: {used} of the {U.UNIT_IW}-wide interior "
      f"({100 * used / U.UNIT_IW:.0f}%)")

# how many rows of the arm band are used at all
band = range(U.R_ARG, U.R_COLLECT)
dead_rows = [y for y in band if not any((x, y) in cells for x in range(U.UNIT_IW + 1))]
print(f"rows in the arm band {U.R_ARG}..{U.R_COLLECT - 1} with no cell at all: {dead_rows}")

print("\nper-row occupancy across the arm band:")
for y in band:
    xs = [x for x in range(U.UNIT_IW + 2) if (x, y) in cells]
