...
def build(
    ...
    * ``word`` above 6: the CPU out-consumes the drum, empties the corridor part
    ...
    g.room(0, 0, cw + 1, 8 + disc + reads - 1)  # the discard sets the depth
    ...
    # ``reads`` is the k-sentinel protocol: the drum emits k sentinels in a row,
    # so one `X` can cover k reads and the loop's per-word cost falls from
    # (6 + 2)/1 to (6 + 2k)/k. k=1 is the shipped loop, glyph for glyph.
    if reads < 1:
        raise ValueError("the loop reads at least one word a lap")
    sh = reads - 1                       # every row below the loop moves down
    if lap == 6:
    ...
    for i in range(sh):                  # the extra reads one `X` will cover
        g.run(2, 4 + i, "^r")
    g.run(2, 4 + sh, "^Xrbv")            # sign test; A<0 goes east to the remainder
    g.run(2, 5 + sh, "^<")
    g.put(6, 5 + sh, ".")
    # The remainder discard: ``disc`` reads a lap, so ``2 * disc + 3`` cells for
    # ``disc`` words. disc=2 is the shipped 2x4.
    g.run(1, 6 + sh, "^a<..<")           # the discard loop's entry
    for i in range(disc):
        g.run(2, 7 + sh + i, "rm")
    g.run(2, 7 + sh + disc, ">^")
    g.run(1, 2, "." * (4 + sh), 0, 1)    # the riser back to the taken row
    ...
    # The protocol change: k sentinels in a row, so whichever of the loop's k
    # reads lands last in the run, the `X` after it still sees a negative. k
    # consecutive sentinels are exactly enough — the first k-window whose end
    # reaches the run ends inside it.
    answer = ("r" + "." * latency + (literal(-1) + "s") * reads
              + literal(rem) + "s")
    if word < 2:
...
def measure(rows: list[str], ticks: int = 400_000, reads: int = 1) -> dict:
    ...
    sh = reads - 1                     # the loop's extra reads push everything down
    parts = {
        "walk": lambda c: cpu(c) and c[1] == 2 and c[0] >= 4,
        "flush": lambda c: cpu(c) and 2 <= c[0] <= 3 and 2 <= c[1] <= 5 + sh,
        "sentinel": lambda c: cpu(c) and 4 <= c[0] <= 6 and 4 + sh <= c[1] <= 6 + sh,
        "discard": lambda c: cpu(c) and 2 <= c[0] <= 3 and 6 + sh <= c[1] <= 9 + sh,
...
def main(argv: list[str]) -> int:
    ...
    ap.add_argument("--reads", type=int, nargs="+", default=[1],
                    help="k-sentinel protocol: words the loop reads per test")
    ...
    if a.dump:
        print("\n".join(build(a.walk[0], a.latency[0], a.corridor[0], a.rem,
                              a.word[0], a.lap[0], a.disc[0], a.reads[0])))
        return 0
                ...
                for corridor in a.corridor:
                    for latency in a.latency:
                      for reads in a.reads:
                        for walk in a.walk:
                            r = measure(
                                build(walk, latency, corridor, a.rem, word, lap,
                                      disc, reads), a.ticks, reads
                            )
                            head = (f"{walk:>5} {latency:>5} {corridor:>5} {word:>5} "
                                    f"{lap:>4} {disc:>4} {reads:>3} "
                                    f"{(6 + 2 * reads) / reads:>5.1f} |")
                            if r["fatal"]:
