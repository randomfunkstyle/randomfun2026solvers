"""Shared harness for the seek-economics probe (worktree-absolute paths)."""
from __future__ import annotations
import sys, time
from pathlib import Path

WT = Path("/Users/ptaykalo/Projects/icfpc-2026/randomfun2026solvers/"
          ".claude/worktrees/agent-aba34ff17375e784e")
...
def run(m, inp, frames, tag=""):
    ...
    print(f"  {tag}: {m.width}x{m.height} fatal={res.fatal} passed={res.passed} "
          f"step={res.step:,} last_frame={res.frame_ticks[-1]:,} ({dt:.0f}s)", flush=True)
    return res
