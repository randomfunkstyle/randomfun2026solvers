"""Dense 29x29 AST brackets parser."""
...
import pytest
from randomfun2026solvers.brackets_dense import build, build_ast, debug_map
...
SOLUTION = ROOT / "tasks" / "solutions" / "brackets_dense.man"
DEBUG_HTML = ROOT / "littleman" / "examples" / "brackets-dense.debug.html"
DEBUG_JSON = ROOT / "littleman" / "examples" / "brackets-dense.debug.json"
...
def test_dense_footprint_and_pipe_bindings() -> None:
    rows = build()
    assert (max(map(len, rows)), len(rows)) == (29, 29)
