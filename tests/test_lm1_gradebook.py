...
def test_checked_in_grid_matches_the_generator() -> None:
    ...
    assert (m.width, m.height) == (108, 88)
    ...
    # Still width-bound, and 108 columns is what this generator emits; the height
    # came down 100 → 88 (slab packing, then dropping the blank interior row above
    # the south wall), which the footprint does not see.
...
def test_checked_in_grid_keeps_the_recorded_shape() -> None:
    ...
    assert (max(map(len, rows)), len(rows)) == (108, 88)
