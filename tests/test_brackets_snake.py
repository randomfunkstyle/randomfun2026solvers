"""Snake-folded AST brackets parser."""
...
import pytest
from randomfun2026solvers.brackets_snake import build, build_ast, debug_map
...
SOLUTION = ROOT / "tasks" / "solutions" / "brackets_snake.man"
DEBUG_HTML = ROOT / "littleman" / "examples" / "brackets-snake.debug.html"
DEBUG_JSON = ROOT / "littleman" / "examples" / "brackets-snake.debug.json"
...
def test_snake_footprint_and_pipe_bindings() -> None:
    rows = build()
    assert (max(map(len, rows)), len(rows)) == (32, 32)
